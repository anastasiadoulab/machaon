...
int main(int argc, char *argv[])
{
    ...
    int    chain1_id =-1;     // select chain_id of splitted chain_1
    int    chain2_id =-1;     // select chain_id of splitted chain_2
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        else if ( !strcmp(argv[i],"-chain1_id") && i < (argc-1) )
        {
            chain1_id=atoi(argv[i + 1]); i++;
        }
        else if ( !strcmp(argv[i],"-chain2_id") && i < (argc-1) )
        {
            chain2_id=atoi(argv[i + 1]); i++;
        }
    ...
    */
    ...
    for (i=0;i<chain1_list.size();i++)
    {
        ...
        {
            if(chain1_id != -1 && chain_i != chain1_id)
                continue;
            ...
            for (j=(dir_opt.size()>0)*(i+1);j<chain2_list.size();j++)
            {
                ...
                {
                    if(chain2_id != -1 && chain_j !=  chain2_id)
                        continue;
